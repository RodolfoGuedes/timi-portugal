document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hero) {
    ...
    if (copy) copy.innerHTML = `
      ...
      <div class="hero-modalities-strip" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:22px 0;">
        <div><small>T2 • 230 USDT</small><strong>5 USDT/dia*</strong></div>
  ...
  if (!document.querySelector('#resumo')) {
    ...
    quick.innerHTML = `<div class="container"><div class="section-heading"><span>01</span><h2>TIMI em 30 segundos</h2></div><p class="lead section-lead">Primeiro, veja o percurso completo. Depois pode aprofundar cada etapa.</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px;margin-top:22px;"><div class="reward-card"><i>1️⃣</i><h3>Escolha</h3><p>Conheça T2, T3 ou T4 e as condições de cada modalidade.</p></div><div class="reward-card"><i>2️⃣</i><h3>Acompanhe</h3><p>Nos dias elegíveis, acompanhe o código disponibilizado nos canais informados.</p></div><div class="reward-card"><i>3️⃣</i><h3>Valide</h3><p>Insira o código na aplicação dentro de uma das janelas disponíveis.</p></div><div class="reward-card"><i>4️⃣</i><h3>Acumule</h3><p>O valor diário indicado é contabilizado quando as condições aplicáveis são cumpridas.</p></div><div class="reward-card"><i>5️⃣</i><h3>Levante</h3><p>Ao atingir o mínimo, pode solicitar levantamento, sujeito à taxa informada de 25%.</p></div></div><div class="financial-note" style="margin-top:18px;"><b>Informação-chave</b><span>A indicação de outras pessoas é opcional e não é requisito para o valor diário da sua modalidade.</span></div></div>`;
